...
def compute_synergy_loss(results, mean_control, high_umi_idx, subtype="POTENTIATION"):
    ...
    linear_params = get_linear_params(pred_res, high_umi_idx, mean_control, all_perts)
    synergy_loss = np.sum(
        [
            np.abs(linear_params[k]["pred"]["mag"] - linear_params[k]["truth"]["mag"])
            for k in linear_params
        ]
    )

    mag = np.sum([np.abs(linear_params[k]["pred"]["mag"]) for k in linear_params])
    return synergy_loss, mag


def non_zero_analysis(adata, test_res):
    metric2fct = {"pearson": pearsonr, "mse": mse}
    ...
    for pert in np.unique(test_res["pert_cat"]):
        ...
        de_idx = [
            geneid2idx[i]
            for i in adata.uns["top_non_zero_de_20"][pert2pert_full_id[pert]]
        ]

        direc_change = np.abs(
            np.sign(test_res["pred"][pert_idx].mean(0)[de_idx] - ctrl[0][de_idx])
            - np.sign(test_res["truth"][pert_idx].mean(0)[de_idx] - ctrl[0][de_idx])
        )
        frac_correct_direction = len(np.where(direc_change == 0)[0]) / len(de_idx)
        pert_metric[pert][
            "frac_correct_direction_top20_non_zero"
        ] = frac_correct_direction

        frac_direction_opposite = len(np.where(direc_change == 2)[0]) / len(de_idx)
        pert_metric[pert][
            "frac_opposite_direction_top20_non_zero"
        ] = frac_direction_opposite
        ...
        if len(nonzero_des) == 0:
            ...
            pred_mean = np.mean(test_res["pred"][pert_idx][:, de_idx], axis=0).reshape(
                -1,
            )
            true_mean = np.mean(test_res["truth"][pert_idx][:, de_idx], axis=0).reshape(
                -1,
            )

            in_range = (pred_mean[nonzero_des] >= min_[nonzero_des]) & (
                pred_mean[nonzero_des] <= max_[nonzero_des]
            )
            frac_in_range = sum(in_range) / len(nonzero_des)
            pert_metric[pert]["frac_in_range_non_zero"] = frac_in_range

            in_range_5 = (pred_mean[nonzero_des] >= q45[nonzero_des]) & (
                pred_mean[nonzero_des] <= q55[nonzero_des]
            )
            frac_in_range_45_55 = sum(in_range_5) / len(nonzero_des)
            pert_metric[pert]["frac_in_range_45_55_non_zero"] = frac_in_range_45_55

            in_range_10 = (pred_mean[nonzero_des] >= q40[nonzero_des]) & (
                pred_mean[nonzero_des] <= q60[nonzero_des]
            )
            frac_in_range_40_60 = sum(in_range_10) / len(nonzero_des)
            pert_metric[pert]["frac_in_range_40_60_non_zero"] = frac_in_range_40_60

            in_range_25 = (pred_mean[nonzero_des] >= q25[nonzero_des]) & (
                pred_mean[nonzero_des] <= q75[nonzero_des]
            )
            ...
            pert_metric[pert]["frac_sigma_below_1_non_zero"] = 1 - len(
                np.where(sigma > 1)[0]
            ) / len(zero_idx)
            pert_metric[pert]["frac_sigma_below_2_non_zero"] = 1 - len(
                np.where(sigma > 2)[0]
            ) / len(zero_idx)
        ...
        for m, fct in metric2fct.items():
            if m != "mse":
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx] - ctrl[0][de_idx],
                ...
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx],
                    test_res["truth"][p_idx].mean(0)[de_idx],
                )[0]
                if np.isnan(val):
                    val = 0
...
def non_dropout_analysis(adata, test_res):
    ...
    for pert in np.unique(test_res["pert_cat"]):
        ...
        de_idx = [
            geneid2idx[i]
            for i in adata.uns["top_non_dropout_de_20"][pert2pert_full_id[pert]]
        ]
        non_zero_idx = adata.uns["non_zeros_gene_idx"][pert2pert_full_id[pert]]
        non_dropout_gene_idx = adata.uns["non_dropout_gene_idx"][
            pert2pert_full_id[pert]
        ]
        ...
        frac_direction_opposite = len(np.where(direc_change == 1)[0]) / len(de_idx)
        pert_metric[pert][
            "frac_0/1_direction_top20_non_dropout"
        ] = frac_direction_opposite

        direc_change = np.abs(
            np.sign(
                test_res["pred"][pert_idx].mean(0)[non_zero_idx] - ctrl[0][non_zero_idx]
            )
            - np.sign(
                test_res["truth"][pert_idx].mean(0)[non_zero_idx]
                - ctrl[0][non_zero_idx]
            )
        )
        ...
        frac_direction_opposite = len(np.where(direc_change == 2)[0]) / len(
            non_zero_idx
        )
        pert_metric[pert]["frac_opposite_direction_non_zero"] = frac_direction_opposite

        frac_direction_opposite = len(np.where(direc_change == 1)[0]) / len(
            non_zero_idx
        )
        ...
        direc_change = np.abs(
            np.sign(
                test_res["pred"][pert_idx].mean(0)[non_dropout_gene_idx]
                - ctrl[0][non_dropout_gene_idx]
            )
            - np.sign(
                test_res["truth"][pert_idx].mean(0)[non_dropout_gene_idx]
                - ctrl[0][non_dropout_gene_idx]
            )
        )
        frac_correct_direction = len(np.where(direc_change == 0)[0]) / len(
        ...
        pert_metric[pert][
            "frac_opposite_direction_non_dropout"
        ] = frac_direction_opposite
        ...
        if len(nonzero_des) == 0:
            ...
            pert_metric[pert]["frac_sigma_below_1_non_dropout"] = 1 - len(
                np.where(sigma > 1)[0]
            ) / len(zero_idx)
            pert_metric[pert]["frac_sigma_below_2_non_dropout"] = 1 - len(
                np.where(sigma > 2)[0]
            ) / len(zero_idx)
...
def deeper_analysis(
    adata, test_res, de_column_prefix="rank_genes_groups_cov", most_variable_genes=None
):
    ...
    for pert in np.unique(test_res["pert_cat"]):
        ...
        de_idx = [
            geneid2idx[i]
            for i in adata.uns["rank_genes_groups_cov_all"][pert2pert_full_id[pert]][
                :20
            ]
        ]
        de_idx_200 = [
        ...
        pert_idx = np.where(test_res["pert_cat"] == pert)[0]
        pred_mean = np.mean(test_res["pred_de"][pert_idx], axis=0).reshape(
            -1,
        )
        true_mean = np.mean(test_res["truth_de"][pert_idx], axis=0).reshape(
            -1,
        )

        direc_change = np.abs(
            np.sign(test_res["pred"][pert_idx].mean(0) - ctrl[0])
        ...
        for val in [20, 50, 100, 200]:
            ...
            direc_change = np.abs(
                np.sign(
                    test_res["pred"][pert_idx].mean(0)[de_idx_map[val]]
                    - ctrl[0][de_idx_map[val]]
                )
                - np.sign(
                    test_res["truth"][pert_idx].mean(0)[de_idx_map[val]]
                    - ctrl[0][de_idx_map[val]]
                )
            )
            frac_correct_direction = len(np.where(direc_change == 0)[0]) / val
            pert_metric[pert][
                "frac_correct_direction_" + str(val)
            ] = frac_correct_direction
        ...
        if len(nonzero_des) == 0:
            ...
            direc_change = np.abs(
                np.sign(pred_mean[nonzero_des] - ctrl[0][de_idx][nonzero_des])
            ...
            pert_metric[pert][
                "frac_correct_direction_20_nonzero"
            ] = frac_correct_direction
            ...
            pert_metric[pert]["frac_sigma_below_1"] = 1 - len(
                np.where(sigma > 1)[0]
            ) / len(zero_idx)
            pert_metric[pert]["frac_sigma_below_2"] = 1 - len(
                np.where(sigma > 2)[0]
            ) / len(zero_idx)
        ...
        for m, fct in metric2fct.items():
            if m != "mse":
                val = fct(
                    test_res["pred"][p_idx].mean(0) - ctrl[0],
                    test_res["truth"][p_idx].mean(0) - ctrl[0],
                )[0]
                if np.isnan(val):
                    val = 0
        ...
        ## up fold changes > 10?
        pert_mean = np.mean(test_res["truth"][p_idx], axis=0).reshape(
            -1,
        )
        ...
        if len(o) > 0:
            pert_metric[pert]["fold_change_gap_all"] = np.mean(
                np.abs(pred_fc / ctrl_fc - true_fc / ctrl_fc)
            )

        o = np.intersect1d(
            np.where(fold_change[0] < 0.333)[0], np.where(fold_change[0] > 0)[0]
        )
        ...
        if len(o) > 0:
            pert_metric[pert]["fold_change_gap_downreg_0.33"] = np.mean(
                np.abs(pred_fc / ctrl_fc - true_fc / ctrl_fc)
            )

        o = np.intersect1d(
            np.where(fold_change[0] < 0.1)[0], np.where(fold_change[0] > 0)[0]
        )
        ...
        if len(o) > 0:
            pert_metric[pert]["fold_change_gap_downreg_0.1"] = np.mean(
                np.abs(pred_fc / ctrl_fc - true_fc / ctrl_fc)
            )
        ...
        if len(o) > 0:
            pert_metric[pert]["fold_change_gap_upreg_3"] = np.mean(
                np.abs(pred_fc / ctrl_fc - true_fc / ctrl_fc)
            )
        ...
        if len(o) > 0:
            pert_metric[pert]["fold_change_gap_upreg_10"] = np.mean(
                np.abs(pred_fc / ctrl_fc - true_fc / ctrl_fc)
            )
        ...
        for m, fct in metric2fct.items():
            if m != "mse":
                val = fct(
                    test_res["pred"][p_idx].mean(0)[most_variable_genes]
                    - ctrl[0][most_variable_genes],
        ...
        for m, fct in metric2fct.items():
            if m != "mse":
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx_200] - ctrl[0][de_idx_200],
                ...
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx_200],
                    test_res["truth"][p_idx].mean(0)[de_idx_200],
                )[0]
                if np.isnan(val):
                    val = 0
        ...
        for m, fct in metric2fct.items():
            if m != "mse":
                ...
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx_100] - ctrl[0][de_idx_100],
                ...
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx_100],
                    test_res["truth"][p_idx].mean(0)[de_idx_100],
                )[0]
                if np.isnan(val):
                    val = 0
        ...
        for m, fct in metric2fct.items():
            if m != "mse":
                ...
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx_50] - ctrl[0][de_idx_50],
                ...
                val = fct(
                    test_res["pred"][p_idx].mean(0)[de_idx_50],
                    test_res["truth"][p_idx].mean(0)[de_idx_50],
                )[0]
                if np.isnan(val):
                    val = 0
    ...
    for GI_type, gi_list in GIs.items():
        ...
        if len(intersect) != 0:
            ...
            for m in list(list(pert_metric.values())[0].keys()):
                GI_type2Score[GI_type][m] = np.mean(
                    [pert_metric[i][m] for i in intersect if m in pert_metric[i]]
                )
...
def get_mean_ctrl(adata):
    return adata[adata.obs["condition"] == "ctrl"].to_df().mean().reset_index(drop=True)
...
def get_test_set_results_seen2(res, sel_GI_type):
    ...
    test_pert_cats = [
        p for p in np.unique(res["pert_cat"]) if p in GIs[sel_GI_type] or "ctrl" in p
    ]
...
def get_all_vectors(all_res, mean_control, double, single1, single2, high_umi_idx):
    # Pred
    ...
    return {
        "single_pred_1": delta_single_pred_1.values[high_umi_idx],
...
def Fit(all_vectors, type_="pertnet"):
    if type_ == "pertnet":
        singles_expr = np.array(
            [all_vectors["single_pred_1"], all_vectors["single_pred_2"]]
        ).T
        ...
        singles_expr = np.array(
            [all_vectors["single_truth_1"], all_vectors["single_truth_2"]]
        ).T
...
def get_linear_params(pred_res, high_umi_idx, mean_control, all_perts):
    ...
    for c in set(pred_res["pert_cat"]):
        ...
        all_vectors = get_all_vectors(
            pred_res, mean_control, double, single1, single2, high_umi_idx
        )

        pertnet_results = Fit(all_vectors, type_="pertnet")
        truth_results = Fit(all_vectors, type_="truth")

        results[c] = {"truth": truth_results, "pred": pertnet_results}
