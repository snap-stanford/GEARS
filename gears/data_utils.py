...
class DataSplitter:
        ...
        if only_test_set_perts and (test_set_perts is not None):
            ...
            if test_set_perts is not None:
                ...
                ood_genes_exclude_test_set = np.setdiff1d(
                    unique_pert_genes, np.union1d(train_gene_candidates, test_set_perts)
                )
                train_set_addition = np.random.choice(
                    ood_genes_exclude_test_set, num_overlap, replace=False
                )
                train_gene_candidates = np.concatenate(
                    (train_gene_candidates, train_set_addition)
                )
    ...
    def get_simulation_split(
        ...
        pert_combo = self.get_perts_from_genes(
            train_gene_candidates, pert_list, "combo"
        )
        ...
        pert_combo_train = np.random.choice(
            pert_combo, int(len(pert_combo) * combo_seen2_train_frac), replace=False
        )
    ...
    def get_split_list(
        ...
        if test_pert_genes is None:
            test_pert_genes = np.random.choice(
                unique_pert_genes, int(len(single_perts) * test_size)
            )
        ...
        if self.split_type == "single" or self.split_type == "single_only":
            ...
            if self.split_type == "single_only":
                ...
                hold_out = self.get_perts_from_genes(
                    test_pert_genes, pert_list, "combo"
                )
            ...
            if self.seen == 0:
                ...
                single_perts = self.get_perts_from_genes(
                    test_pert_genes, pert_list, "single"
                )
                combo_perts = self.get_perts_from_genes(
                    test_pert_genes, pert_list, "combo"
                )
                ...
                if test_perts is None:
                    test_perts = np.random.choice(
                        combo_perts, int(len(combo_perts) * test_size)
                    )
                else:
