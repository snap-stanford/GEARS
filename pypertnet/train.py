'''
import PertNet, PertNet_Data

data = PertNet_Data(dataset = ‘Norman2019’, split = 'simulation', seed = 1)

model = PertNet(uncertainty = False,
                perturb_graph = 'GO', 
                gene_graph = 'Co-express',
                sim_gene_k = 20,
                device = 'cuda:0')
...
model.save('XXX')
model.load_pretrained('XXX')
model.predict(genes = [A, B])
...
def parse_arguments():
    ...
    # model arguments
    parser.add_argument('--node_hidden_size', type=int, default=64, help='hidden dimension for GNN')    
    parser.add_argument('--model', choices = ['PertNet'], type = str, default = 'PertNet', help='model name')
